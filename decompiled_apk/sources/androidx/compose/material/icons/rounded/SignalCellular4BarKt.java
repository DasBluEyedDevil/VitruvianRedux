package androidx.compose.material.icons.rounded;

import androidx.compose.p000ui.graphics.vector.ImageVector;
import kotlin.Metadata;

/* compiled from: SignalCellular4Bar.kt */
@Metadata(m145d1 = {"\u0000\u0010\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\"\u0010\u0010\u0000\u001a\u0004\u0018\u00010\u0001X\u0082\u000e¢\u0006\u0002\n\u0000\"\u0015\u0010\u0002\u001a\u00020\u0001*\u00020\u00038F¢\u0006\u0006\u001a\u0004\b\u0004\u0010\u0005¨\u0006\u0006"}, m146d2 = {"_signalCellular4Bar", "Landroidx/compose/ui/graphics/vector/ImageVector;", "SignalCellular4Bar", "Landroidx/compose/material/icons/Icons$Rounded;", "getSignalCellular4Bar", "(Landroidx/compose/material/icons/Icons$Rounded;)Landroidx/compose/ui/graphics/vector/ImageVector;", "material-icons-extended_release"}, m147k = 2, m148mv = {1, 8, 0}, m150xi = 48)
/* loaded from: classes.dex */
public final class SignalCellular4BarKt {
    private static ImageVector _signalCellular4Bar;

    /*  JADX ERROR: JadxRuntimeException in pass: InlineMethods
        jadx.core.utils.exceptions.JadxRuntimeException: Failed to process method for inline: androidx.compose.ui.graphics.vector.ImageVector.Builder.addPath-oIyEayM$default(androidx.compose.ui.graphics.vector.ImageVector$Builder, java.util.List, int, java.lang.String, androidx.compose.ui.graphics.Brush, float, androidx.compose.ui.graphics.Brush, float, float, int, int, float, float, float, float, int, java.lang.Object):androidx.compose.ui.graphics.vector.ImageVector$Builder
        	at jadx.core.dex.visitors.InlineMethods.processInvokeInsn(InlineMethods.java:74)
        	at jadx.core.dex.visitors.InlineMethods.visit(InlineMethods.java:49)
        Caused by: java.lang.NullPointerException
        */
    public static final androidx.compose.p000ui.graphics.vector.ImageVector getSignalCellular4Bar(androidx.compose.material.icons.Icons.Rounded r33) {
        /*
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.rounded.SignalCellular4BarKt._signalCellular4Bar
            if (r0 == 0) goto La
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.rounded.SignalCellular4BarKt._signalCellular4Bar
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        La:
            java.lang.String r2 = "Rounded.SignalCellular4Bar"
            r10 = 0
            r0 = 0
            androidx.compose.ui.graphics.vector.ImageVector$Builder r1 = new androidx.compose.ui.graphics.vector.ImageVector$Builder
            r3 = 1103101952(0x41c00000, float:24.0)
            r4 = 0
            float r3 = androidx.compose.p000ui.unit.C0897Dp.m8629constructorimpl(r3)
            r4 = 1103101952(0x41c00000, float:24.0)
            r5 = 0
            float r4 = androidx.compose.p000ui.unit.C0897Dp.m8629constructorimpl(r4)
            r11 = 96
            r12 = 0
            r5 = 1103101952(0x41c00000, float:24.0)
            r6 = 1103101952(0x41c00000, float:24.0)
            r7 = 0
            r9 = 0
            r1.<init>(r2, r3, r4, r5, r6, r7, r9, r10, r11, r12)
            r3 = 0
            r4 = r1
            r16 = 1065353216(0x3f800000, float:1.0)
            r18 = 1065353216(0x3f800000, float:1.0)
            int r13 = androidx.compose.p000ui.graphics.vector.VectorKt.getDefaultFillType()
            r5 = 0
            androidx.compose.ui.graphics.SolidColor r6 = new androidx.compose.ui.graphics.SolidColor
            androidx.compose.ui.graphics.Color$Companion r7 = androidx.compose.p000ui.graphics.Color.INSTANCE
            long r7 = r7.m5911getBlack0d7_KjU()
            r9 = 0
            r6.<init>(r7, r9)
            r15 = r6
            androidx.compose.ui.graphics.Brush r15 = (androidx.compose.p000ui.graphics.Brush) r15
            androidx.compose.ui.graphics.StrokeCap$Companion r6 = androidx.compose.p000ui.graphics.StrokeCap.INSTANCE
            int r20 = r6.m6255getButtKaPHkGw()
            androidx.compose.ui.graphics.StrokeJoin$Companion r6 = androidx.compose.p000ui.graphics.StrokeJoin.INSTANCE
            int r21 = r6.m6265getBevelLxFBmk8()
            r17 = r9
            r11 = r4
            r19 = 1065353216(0x3f800000, float:1.0)
            r22 = r19
            java.lang.String r14 = ""
            r6 = 0
            r7 = 0
            androidx.compose.ui.graphics.vector.PathBuilder r8 = new androidx.compose.ui.graphics.vector.PathBuilder
            r8.<init>()
            r9 = 0
            r12 = r8
            r30 = 0
            r31 = r0
            r0 = 1102053376(0x41b00000, float:22.0)
            r32 = r1
            r1 = 1082990264(0x408d1eb8, float:4.41)
            r12.moveTo(r1, r0)
            r0 = 1101004800(0x41a00000, float:20.0)
            r12.horizontalLineTo(r0)
            r28 = 1073741824(0x40000000, float:2.0)
            r29 = -1073741824(0xffffffffc0000000, float:-2.0)
            r24 = 1066192077(0x3f8ccccd, float:1.1)
            r25 = 0
            r26 = 1073741824(0x40000000, float:2.0)
            r27 = -1083808154(0xffffffffbf666666, float:-0.9)
            r23 = r12
            r23.curveToRelative(r24, r25, r26, r27, r28, r29)
            r0 = r23
            r0.verticalLineTo(r1)
            r28 = -1076174520(0xffffffffbfdae148, float:-1.71)
            r29 = -1086995825(0xffffffffbf35c28f, float:-0.71)
            r24 = 0
            r25 = -1083975926(0xffffffffbf63d70a, float:-0.89)
            r26 = -1081459343(0xffffffffbf8a3d71, float:-1.08)
            r27 = -1079278305(0xffffffffbfab851f, float:-1.34)
            r23.curveToRelative(r24, r25, r26, r27, r28, r29)
            r1 = 1080914084(0x406d70a4, float:3.71)
            r12 = 1101156844(0x41a251ec, float:20.29)
            r0.lineTo(r1, r12)
            r28 = 1060320051(0x3f333333, float:0.7)
            r29 = 1071309128(0x3fdae148, float:1.71)
            r24 = -1088338002(0xffffffffbf2147ae, float:-0.63)
            r25 = 1059145646(0x3f2147ae, float:0.63)
            r26 = -1102934180(0xffffffffbe428f5c, float:-0.19)
            r27 = 1071309128(0x3fdae148, float:1.71)
            r23.curveToRelative(r24, r25, r26, r27, r28, r29)
            r23.close()
            java.util.List r12 = r8.getNodes()
            r26 = 14336(0x3800, float:2.0089E-41)
            r27 = 0
            r23 = 0
            r24 = 0
            r25 = 0
            androidx.compose.ui.graphics.vector.ImageVector$Builder r0 = androidx.compose.ui.graphics.vector.ImageVector.Builder.m6621addPathoIyEayM$default(r11, r12, r13, r14, r15, r16, r17, r18, r19, r20, r21, r22, r23, r24, r25, r26, r27)
            androidx.compose.ui.graphics.vector.ImageVector r0 = r0.build()
            androidx.compose.material.icons.rounded.SignalCellular4BarKt._signalCellular4Bar = r0
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.rounded.SignalCellular4BarKt._signalCellular4Bar
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: androidx.compose.material.icons.rounded.SignalCellular4BarKt.getSignalCellular4Bar(androidx.compose.material.icons.Icons$Rounded):androidx.compose.ui.graphics.vector.ImageVector");
    }
}
