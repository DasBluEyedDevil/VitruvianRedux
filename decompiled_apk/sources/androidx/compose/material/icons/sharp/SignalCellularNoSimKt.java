package androidx.compose.material.icons.sharp;

import androidx.compose.p000ui.graphics.vector.ImageVector;
import kotlin.Metadata;

/* compiled from: SignalCellularNoSim.kt */
@Metadata(m145d1 = {"\u0000\u0010\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\"\u0010\u0010\u0000\u001a\u0004\u0018\u00010\u0001X\u0082\u000e¢\u0006\u0002\n\u0000\"\u0015\u0010\u0002\u001a\u00020\u0001*\u00020\u00038F¢\u0006\u0006\u001a\u0004\b\u0004\u0010\u0005¨\u0006\u0006"}, m146d2 = {"_signalCellularNoSim", "Landroidx/compose/ui/graphics/vector/ImageVector;", "SignalCellularNoSim", "Landroidx/compose/material/icons/Icons$Sharp;", "getSignalCellularNoSim", "(Landroidx/compose/material/icons/Icons$Sharp;)Landroidx/compose/ui/graphics/vector/ImageVector;", "material-icons-extended_release"}, m147k = 2, m148mv = {1, 8, 0}, m150xi = 48)
/* loaded from: classes.dex */
public final class SignalCellularNoSimKt {
    private static ImageVector _signalCellularNoSim;

    /*  JADX ERROR: JadxRuntimeException in pass: InlineMethods
        jadx.core.utils.exceptions.JadxRuntimeException: Failed to process method for inline: androidx.compose.ui.graphics.vector.ImageVector.Builder.addPath-oIyEayM$default(androidx.compose.ui.graphics.vector.ImageVector$Builder, java.util.List, int, java.lang.String, androidx.compose.ui.graphics.Brush, float, androidx.compose.ui.graphics.Brush, float, float, int, int, float, float, float, float, int, java.lang.Object):androidx.compose.ui.graphics.vector.ImageVector$Builder
        	at jadx.core.dex.visitors.InlineMethods.processInvokeInsn(InlineMethods.java:74)
        	at jadx.core.dex.visitors.InlineMethods.visit(InlineMethods.java:49)
        Caused by: java.lang.NullPointerException
        */
    public static final androidx.compose.p000ui.graphics.vector.ImageVector getSignalCellularNoSim(androidx.compose.material.icons.Icons.Sharp r30) {
        /*
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.sharp.SignalCellularNoSimKt._signalCellularNoSim
            if (r0 == 0) goto La
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.sharp.SignalCellularNoSimKt._signalCellularNoSim
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        La:
            java.lang.String r2 = "Sharp.SignalCellularNoSim"
            r10 = 0
            r0 = 0
            androidx.compose.ui.graphics.vector.ImageVector$Builder r1 = new androidx.compose.ui.graphics.vector.ImageVector$Builder
            r3 = 1103101952(0x41c00000, float:24.0)
            r4 = 0
            float r3 = androidx.compose.p000ui.unit.C0897Dp.m8629constructorimpl(r3)
            r4 = 1103101952(0x41c00000, float:24.0)
            r5 = 0
            float r4 = androidx.compose.p000ui.unit.C0897Dp.m8629constructorimpl(r4)
            r11 = 96
            r12 = 0
            r5 = 1103101952(0x41c00000, float:24.0)
            r6 = 1103101952(0x41c00000, float:24.0)
            r7 = 0
            r9 = 0
            r1.<init>(r2, r3, r4, r5, r6, r7, r9, r10, r11, r12)
            r3 = 0
            r4 = r1
            r16 = 1065353216(0x3f800000, float:1.0)
            r18 = 1065353216(0x3f800000, float:1.0)
            int r13 = androidx.compose.p000ui.graphics.vector.VectorKt.getDefaultFillType()
            r5 = 0
            androidx.compose.ui.graphics.SolidColor r6 = new androidx.compose.ui.graphics.SolidColor
            androidx.compose.ui.graphics.Color$Companion r7 = androidx.compose.p000ui.graphics.Color.INSTANCE
            long r7 = r7.m5911getBlack0d7_KjU()
            r9 = 0
            r6.<init>(r7, r9)
            r15 = r6
            androidx.compose.ui.graphics.Brush r15 = (androidx.compose.p000ui.graphics.Brush) r15
            androidx.compose.ui.graphics.StrokeCap$Companion r6 = androidx.compose.p000ui.graphics.StrokeCap.INSTANCE
            int r20 = r6.m6255getButtKaPHkGw()
            androidx.compose.ui.graphics.StrokeJoin$Companion r6 = androidx.compose.p000ui.graphics.StrokeJoin.INSTANCE
            int r21 = r6.m6265getBevelLxFBmk8()
            r17 = r9
            r11 = r4
            r19 = 1065353216(0x3f800000, float:1.0)
            r22 = r19
            java.lang.String r14 = ""
            r6 = 0
            r7 = 0
            androidx.compose.ui.graphics.vector.PathBuilder r8 = new androidx.compose.ui.graphics.vector.PathBuilder
            r8.<init>()
            r9 = 0
            r12 = r8
            r23 = 0
            r28 = r0
            r0 = 1077936128(0x40400000, float:3.0)
            r29 = r1
            r1 = 1100480512(0x41980000, float:19.0)
            r12.moveTo(r1, r0)
            r0 = -1055916032(0xffffffffc1100000, float:-9.0)
            r12.horizontalLineToRelative(r0)
            r0 = 1090414182(0x40fe6666, float:7.95)
            r1 = 1084353413(0x40a1eb85, float:5.06)
            r12.lineTo(r0, r1)
            r0 = 1098965320(0x4180e148, float:16.11)
            r1 = 1100480512(0x41980000, float:19.0)
            r12.lineTo(r1, r0)
            r12.close()
            r0 = 1081249628(0x40728f5c, float:3.79)
            r1 = 1081039913(0x406f5c29, float:3.74)
            r12.moveTo(r0, r1)
            r0 = 1075335660(0x401851ec, float:2.38)
            r1 = 1084542157(0x40a4cccd, float:5.15)
            r12.lineTo(r0, r1)
            r0 = 1090036695(0x40f8a3d7, float:7.77)
            r1 = 1084227584(0x40a00000, float:5.0)
            r12.lineTo(r1, r0)
            r0 = 1101529088(0x41a80000, float:21.0)
            r12.lineTo(r1, r0)
            r0 = 1096003092(0x4153ae14, float:13.23)
            r12.horizontalLineToRelative(r0)
            r0 = 1070554153(0x3fcf5c29, float:1.62)
            r12.lineToRelative(r0, r0)
            r0 = 1068792545(0x3fb47ae1, float:1.41)
            r1 = -1078691103(0xffffffffbfb47ae1, float:-1.41)
            r12.lineToRelative(r0, r1)
            r12.close()
            java.util.List r12 = r8.getNodes()
            r26 = 14336(0x3800, float:2.0089E-41)
            r27 = 0
            r23 = 0
            r24 = 0
            r25 = 0
            androidx.compose.ui.graphics.vector.ImageVector$Builder r0 = androidx.compose.ui.graphics.vector.ImageVector.Builder.m6621addPathoIyEayM$default(r11, r12, r13, r14, r15, r16, r17, r18, r19, r20, r21, r22, r23, r24, r25, r26, r27)
            androidx.compose.ui.graphics.vector.ImageVector r0 = r0.build()
            androidx.compose.material.icons.sharp.SignalCellularNoSimKt._signalCellularNoSim = r0
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.sharp.SignalCellularNoSimKt._signalCellularNoSim
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: androidx.compose.material.icons.sharp.SignalCellularNoSimKt.getSignalCellularNoSim(androidx.compose.material.icons.Icons$Sharp):androidx.compose.ui.graphics.vector.ImageVector");
    }
}
